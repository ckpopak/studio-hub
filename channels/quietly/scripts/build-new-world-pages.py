"""One-off builder: write worlds/<slug>.html for QuietLY episodes 12-15.

Mirrors the structure of worlds/autumn-river-overlook.html so episode-player.js
can render hero, comics, embedded session, and tracklist from data/episode-N.json.
"""
...
WORLDS = os.path.normpath(os.path.join(HERE, "..", "worlds"))
...
<html lang="en">
  <head>
    ...
    <title>{en} · {zh} — Cafe QuietLY 靜</title>
    <meta
      name="description"
      content="QuietLY Episode {ep} field notes: quiet comic, full lyrics from n20dle prompts, and embedded long listening for {zh}."
    />
  ...
  <body data-episode="../data/episode-{ep}.json">
    ...
    <main>
      ...
      <section class="session reveal" id="session" aria-labelledby="session-title">
        ...
        <p class="section__note">
          ...
          Lyric text is taken from <code>songs/prompts/episode-{ep}/*.md</code>.
        </p>
    ...
    <footer class="site-footer">
      <p>
        ...
        <code>n20dle/songs/prompts/episode-{ep}/*.md</code> · Lyrics fenced blocks.
      </p>
...
def main() -> None:
    for e in (12, 13, 14, 15):
        with io.open(
            os.path.join(DATA, f"episode-{e:02d}.json"), encoding="utf-8"
        ) as fh:
            d = json.load(fh)
        ...
        path = os.path.join(WORLDS, f"{d['slug']}.html")
